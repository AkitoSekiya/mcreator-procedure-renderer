...
[Google Blockly](https://developers.google.com/blockly) で機械的に描画し、SVG/PNG画像として保存できる
1画面Webアプリです。画像生成AIは使用していません。GitHub Pagesでの静的配信を前提としています。

## 概要

- 左ペインにプロシージャ構造化JSONを入力（貼り付け／ファイル読込／サンプル選択）
- `blocks_full.json`（検証用マスターデータ）に基づいて厳密に検証し、エラー・警告・情報メッセージを一覧表示
- 検証に成功したら右ペインのBlocklyワークスペースに実際のMCreatorブロックと同じ見た目で描画
- ズーム操作、SVG/PNG（1x〜4x）でのダウンロードが可能
- 画面下部にブロック検索パネル（block_id・日本語名・英語名・カテゴリの部分一致検索）
...
- **SVG保存**: ワークスペースの内容をベクター（文字もアウトライン化せずテキストのまま）でスタンドアロンSVGとして保存
- **PNG保存**: 倍率（1x〜4x）を選択してPNGとして保存。背景は白固定

ファイル名は `procedure_name`（未指定時は `procedure`）を基に、`<name>.svg` / `<name>.png`（等倍）/
`<name>@2x.png` のように付与されます。
...
src/
  ...
  blockly/        # registerBlocks / fields / toXml / workspace / export
  components/     # Toolbar / ValidationList / StatusBar / BlockSearchPanel
