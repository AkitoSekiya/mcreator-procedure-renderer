...
> - **モード判別（v1後方互換）**: 文書全体に node_id 文字列参照が1つでもあれば**グラフモード**（上記の複数スタック描画をそのまま適用）。文字列参照が一切無ければ**レガシーモード**（v1公開契約「blocksの配列順＝上から下のメインのステートメント列」を完全維持: 複数の未参照ルートは配列順に自動でnext連結して1本のメイン列にし、W004は出さない）。判別結果は `ResolvedDoc`/`NormalizedProcedure` の `mode` フィールドに保持（デバッグ用、UI非表示）
...
5. **ルート自動分類とモード判別（v1後方互換）**: どこからも参照されていないノードのうち shape=statement/hat
   をルートとし、その `next` チェーンを展開する。ここで文書を2つのモードに判別する:
   - **グラフモード**（文書中に `value_inputs`/`statement_inputs`/`next` のいずれかで node_id 文字列参照が
     1つでも存在する場合）: 最初のルート（`blocks` 配列順）のチェーンをメイン列としてtriggerに接続。
     2本目以降の未接続チェーンは **W004**「接続されていないステートメント列」を出したうえで、独立した
     スタックとして描画する（Blockly XMLは複数の独立した最上位 `<block>` を正当にサポートする）
   - **レガシーモード**（node_id文字列参照が文書中に一切存在しない、純粋なネスト形式の場合）: v1の公開契約
     「`blocks` 配列＝上から下へ繋がるメインのステートメント列」を完全に維持するため、未参照ルートが複数
     あっても `blocks` 配列の出現順に自動で `next` 連結し、1本のメイン列にする。**W004は出さない**
     （連結は本来のv1の正常動作であり、通知の必要もない）
   - 判別結果（`'graph' | 'legacy'`）は `ResolvedDoc`/`NormalizedProcedure` の `mode` フィールドに保持する
     （デバッグ用。UIには表示しない）
